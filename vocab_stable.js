...
async function guess(){
    ...
    let itt=0;
    while(1){
        if(itt==4){console.log("wtf");break}
        await sleep(1500);
...
async function answer(){
    ...
    if(document.getElementsByClassName("choices")[last_index.choices]!=undefined){
        ...
        if(viable_options.length==0){
            ...
            //!word may be in different form/tense (eg. word=abated,word_list only contains def for abate)
            //TODO: algorithm to get permutations of words/root of permutations
            for(let i=0;i<4;i++){
            ...
            //guess the answer for now
            guess();
            return
        }
        //viable options may be blank if question_type != typeD
    }
